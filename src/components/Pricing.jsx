  ...
  return (
    <section id="pricing" className="relative py-28 lg:py-36 bg-bg-secondary border-t border-border/60">
      <div className="max-w-content mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div ref={cardsRef} className="reveal grid md:grid-cols-3 gap-8 lg:gap-10 items-stretch">
          {data.tiers?.map((tier, i) => (
            <div
              key={i}
              className={`relative rounded-3xl p-8 sm:p-10 flex flex-col justify-between transition-all duration-300 stagger-${i + 1} ${
                tier.recommended
                  ? 'bg-bg-card border-2 border-[#3B7097] shadow-2xl shadow-[#3B7097]/15 scale-[1.03] z-10'
                  : 'bg-bg-card border border-border/80 shadow-xs hover:shadow-xl hover:-translate-y-1'
              }`}
            >
              {/* Popular Badge */}
              {tier.recommended && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-4 py-1.5 bg-[#3B7097] text-white font-body font-bold text-xs uppercase tracking-wider rounded-full shadow-md">
                  <Star size={14} />
                  Most Popular
                </div>
              )}
              ...
              <div>
                ...
                {/* Price Display */}
                <div className="flex items-baseline gap-1.5 mb-8 pb-8 border-b border-border/80">
                ...
                <ul className="space-y-4 mb-10">
                  {tier.features?.map((feature, j) => (
                    <li key={j} className="flex items-start gap-3">
                      <div className="w-5 h-5 rounded-full bg-[#3B7097]/15 text-[#3B7097] flex items-center justify-center flex-shrink-0 mt-0.5">
                        <Check size={13} />
                      </div>
              ...
              <a
                ...
                className={`block w-full text-center py-4 rounded-2xl font-body font-bold text-sm transition-all duration-200 ${
                  tier.recommended
                    ? 'bg-[#3B7097] text-white hover:bg-[#2C5677] shadow-lg shadow-[#3B7097]/25 hover:shadow-xl hover:-translate-y-0.5'
                    : 'bg-bg-secondary border border-border text-ink-primary hover:border-[#3B7097] hover:text-[#3B7097] hover:bg-white'
                }`}
              >
                {tier.ctaText}
              </a>
